...
export class EditTaskComponent implements OnInit {
  ...
  } //Cargo local storage con el editField al refrescar
  ...
  getTaskName() {
    ...
    const task = this.taskService.tasksList.find(
      (task) => task.id === this.taskId
    );

    if (this.myEditTaskForm && this.myEditTaskForm.get('nombre')) {
  ...
  saveLocalStorage(): void {
    localStorage.setItem(
      'editField',
      JSON.stringify(this.myEditTaskForm.value.nombre)
    );
  }
